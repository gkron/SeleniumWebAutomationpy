...
from src.conf import remote_credentials
from src.conf import opera_browser_conf

class DriverFactory():
    
    def __init__(self,browser='chrome',browser_version=None,os_name=None):
    ...
    def run_local(self,os_name,os_version,browser,browser_version):
        ...
        local_driver = None
        if browser.lower() == "ff" or browser.lower() == 'firefox':
            local_driver = webdriver.Firefox(executable_path="D:\eclipse-workspace\Demo1\drivers\geckodriver.exe")    
        elif  browser.lower() == "ie":
            local_driver = webdriver.Ie()
        elif browser.lower() == "chrome":
            local_driver = webdriver.Chrome("D:\eclipse-workspace\Demo1\drivers\chromedriver.exe")
        elif browser.lower() == "opera":
            opera_options = None
            try:
